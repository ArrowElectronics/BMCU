...
#define SPI_BEGIN()  init_SPI();
#define SPI_END() can_spi_end();
...
void init_SPI(); // define CS after construction before begin()
...
/* SPI Chip Select */
#define MCP2518fd_SELECT()         CAN_SPI_CS_LOW();
#define MCP2518fd_UNSELECT()       CAN_SPI_CS_HIGH();
...
/**********************END:mcp2518fd_can_dfs.h *******************************/

void enableTxInterrupt(bool enable); // enable transmit interrupt
...
*/
uint8_t begin(uint32_t speedset, const uint8_t clockset); // init can
...
uint8_t readMsgBufID(uint8_t status, volatile unsigned long *id, volatile uint8_t *ext, volatile uint8_t *rtrr, volatile uint8_t *len, volatile uint8_t *buf); // read buf with object ID
/* wrapper */
...
*/
uint8_t trySendMsgBuf(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t dlc, const uint8_t *buf, uint8_t iTxBuf);
uint8_t sendMsgBuf1(uint8_t status, unsigned long id, uint8_t ext, uint8_t rtr, uint8_t dlc, volatile const uint8_t *buf);
uint8_t sendMsgBuf2(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t dlc, const uint8_t *buf, bool wait_sent);
/* wrapper */
